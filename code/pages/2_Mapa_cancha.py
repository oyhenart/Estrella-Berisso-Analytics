...
# ── Encabezado Personalizado (Único) ───────────────────────────────────────────
st.markdown("""
...
# Dimensiones y referencias de la cancha (StatsBomb / Porcentual)
W, H = 100, 100
AG_X = 12.7; AG_Y = 44.8
AC_X = 4.2;  AC_Y = 20.4
ARCO = 8.1;  CR   = 7.0

# Diccionario de colores para los marcadores
colores = {
    "pase":           "#60A5FA",
    "recuperacion":   "#34D399",
    "perdida":        "#F87171",
...
eventos_sel = st.multiselect("Tipo de evento", eventos_disponibles, default=default_eventos)
...
if num_fecha is not None and "fecha" in df_pases_f.columns:
    df_pases_f = df_pases_f[df_pases_f["fecha"] == num_fecha]

# ── Dibujo y Layout de la Cancha ───────────────────────────────────────────────
def shapes_cancha():
    s = []
...
# ── Gráfico 1: Mapa de Eventos Individuales ────────────────────────────────────
fig = go.Figure()

# Vectorización de líneas de pase
...
# Representación geométrica de puntos de evento
for evento in eventos_sel:
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate=(
            "<b>%{customdata[0]}</b><br>"
            "Evento: " + evento + "<br>"
            "Tiempo: %{customdata[1]:.0f}:%{customdata[2]:.0f}<br>"
...
# ── Gráfico 2: Heatmap de Densidad de Acciones ─────────────────────────────────
...
if not df_filtrado.empty:
    ...
    st.info("No hay datos disponibles para la selección actual para generar el mapa de calor.")