...
with col1:
    ...
    fecha_sel = st.selectbox(
        "Partido",
        ["Todos los partidos"] + [f"Fecha {x}" for x in fechas]
    )
...
c1.metric("Pases",           len(df_filtrado[df_filtrado["Event"] == "pase"]))
c2.metric("Recuperaciones",  len(df_filtrado[df_filtrado["Event"] == "recuperacion"]))
c3.metric("Faltas",          len(df_filtrado[df_filtrado["Event"] == "falta cometida"]))
c4.metric("Remates",         len(df_filtrado[df_filtrado["Event"] == "remate"]))
c5.metric("Goles",           len(df_filtrado[df_filtrado["Event"] == "gol"]))
