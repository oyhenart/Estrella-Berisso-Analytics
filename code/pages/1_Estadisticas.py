...
if jugador_sel != "Todos":
    ...
    if df_j.empty:
        st.warning(f"No se registraron acciones para {jugador_sel} en los partidos seleccionados.")
    else:
...
# ── Vista: todos los jugadores ────────────────────────────────────────────────
    ...
    if not rows:
        st.info("No hay eventos registrados para los filtros seleccionados.")
    else:
        ...
        for posicion in ["Arquero", "Defensor", "Mediocampista", "Delantero"]:
            ...
            # Ajuste de seguridad: Filtrar columnas deseadas asegurando que existan físicamente en el DataFrame
            cols_deseadas = ["Jugador", "Pos.", "PJ"] + [LABEL_METRICAS[m] for m in metricas_pos if m in LABEL_METRICAS]
            cols_mostrar = [c for c in cols_deseadas if c in grupo.columns]
